...
public abstract class DLPythonAbstractContext implements DLPythonContext {
    ...
    private OldPythonKernelBackend getLegacyKernelBackend() throws DLInvalidEnvironmentException {
        final PythonKernel kernel = getKernel();
        if (!(kernel.getBackend() instanceof OldPythonKernelBackend)) {
        ...
        return (OldPythonKernelBackend)kernel.getBackend();
    }
    ...
    public void putDataInKernel(final String name, final TableChunker tableChunker, final int rowsPerChunk,
        ...
        try {
            getLegacyKernelBackend().putData(name, tableChunker, rowsPerChunk,
    ...
    public TableCreator<?> getDataFromKernel(final String name, final TableCreatorFactory tcf,
        final DLCancelable cancelable) throws IOException, DLCanceledExecutionException, DLInvalidEnvironmentException {
        try {
            return getLegacyKernelBackend().getData(name, tcf, new DLCancelableWrappingPythonCancelable(cancelable));
        } catch (final Exception ex) {
            throwNarrowedPythonException(ex);
        }
        // This cannot happen.
